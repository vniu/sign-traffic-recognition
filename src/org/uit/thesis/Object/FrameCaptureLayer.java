...
public abstract class FrameCaptureLayer extends SurfaceView implements SurfaceHolder.Callback, Runnable {
    ...
    public FrameCaptureLayer(Context context) {
        super(context);
        counter=0;
        flag=true;
        mHolder = getHolder();
    ...
    private void savePic(Bitmap bmp){
    	...
    	try {
	        ...
	        fOut.flush();
	        fOut.close();
	        //Log.i(TAG, "Image Path : " + MediaStore.Images.Media.insertImage(ctext.getContentResolver(), file.getAbsolutePath(), file.getName(), file.getName()));
	    }
	    catch (Exception e) {
	        e.printStackTrace();
	    }
